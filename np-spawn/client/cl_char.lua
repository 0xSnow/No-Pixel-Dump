...
function Login.CreatePlayerCharacterPeds(characterModelData,isReset)
    ...
    events:Trigger("np-base:fetchPlayerCharacters", nil, function(data)
        ...
        local function isCharacterBlocked(cid)
            local blocked = false
            if selectedCid ~= nil then
                ...
                if not hasPassed then blocked = true end
            end
            return blocked
        end
        ...
        if #data > 8 then
            ...
            for idx = 9, math.min(#data, getCharLimit()) do
                if not isCharacterBlocked(data[idx].id) then
                    extraCharData[#extraCharData+1] = data[idx]
                end
            end
        end

        for _ = 1, math.min(getCharLimit(), 8) do
