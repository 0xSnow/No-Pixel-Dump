...
RegisterUICallback("np-gallery:jewelryMenuAction", function(data, cb)
    ...
    if data.identify then
        Wait(10)
        local items = GetJewelryInventoryItems()
...
function GetJewelryInventoryItems()
    local items = {}
...
Citizen.CreateThread(function()
    while true do
        ...
        for _,effect in ipairs(appliedEffects) do
            if effect.type and effect.type == 'interval' then
...
AddEventHandler('np-gallery:jewelry:healthRegen', function(pLevel)
    ...
    pLevel = pLevel / 4
    exports['ragdoll']:SetPlayerHealth(math.floor(health + pLevel))
end)
