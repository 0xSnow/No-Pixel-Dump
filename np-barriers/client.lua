...
AddEventHandler('barriers:pickup', function()
	...
	local finished = exports["np-taskbar"]:taskBar(1800, _L("barriers-picking-up", "Picking up barrier"))
...
AddEventHandler('barriers:cone', function()
	...
	local finished = exports["np-taskbar"]:taskBar(1800, _L("barriers-placing-cone", "Placing Cone"))
	if finished == 100 then	
...
AddEventHandler('barriers:sbarrier', function()
    loadAnimDict('anim@narcotics@trash')
    TaskPlayAnim(PlayerPedId(),'anim@narcotics@trash', 'drop_front',0.9, -8, 1700, 49, 3.0, 0, 0, 0)
	local finished = exports["np-taskbar"]:taskBar(1800, _L("barriers-placing-barrier", "Placing Barrier"))
	if finished == 100 then	
		local x,y,z = table.unpack(GetOffsetFromEntityInWorldCoords(PlayerPedId(), 0.0, 2.2, 0.0))
		local heading = GetEntityHeading(PlayerPedId())
...
AddEventHandler('barriers:barrier', function()
    loadAnimDict('anim@narcotics@trash')
    TaskPlayAnim(PlayerPedId(),'anim@narcotics@trash', 'drop_front',0.9, -8, 1700, 49, 3.0, 0, 0, 0)
	local finished = exports["np-taskbar"]:taskBar(1800, _L("barriers-placing-roadblock", "Placing Roadblock"))
	if finished == 100 then
		local x,y,z = table.unpack(GetOffsetFromEntityInWorldCoords(PlayerPedId(), 0.0, 2.2, 0.0))
		...
		TriggerEvent("DoLongHudText", _L("barriers-traffic-blocked", "Traffic Blocked in facing direction."),1)
