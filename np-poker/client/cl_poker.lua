...
RegisterNetEvent("np-casino:poker:updateTableState")
AddEventHandler("np-casino:poker:updateTableState", function(state)
    if state.active then
        exports["np-ui"]:SetUIFocus(true, true)
    end
    exports["np-ui"]:sendAppEvent("poker", state)
end)

RegisterUICallback("np-ui:casinoPokerAction", function(data, cb)
    local result = true
    if data.action == "leave" then
        TriggerEvent("np-poker:playPlayerEvent", {event = "leaveGame", tableNum = currentPokerTable and currentPokerTable or -1})
    else
        result = RPC.execute("np-casino:poker:playerAction", GetPlayerServerId(PlayerId()), myTableId, data.action, data.amount)
    end
...
--     if a == "join" then
--         RPC.execute("np-casino:poker:joinGame", tonumber(b), 3, tonumber(c))
--     end
--     if a == "fold" then
--         RPC.execute("np-casino:poker:playerAction", tonumber(b), 3, a, c)
--     end
--     if a == "leave" then
--         RPC.execute("np-casino:poker:playerAction", tonumber(b), 3, a, c)
--     end
...
RegisterNetEvent("np-poker:givePlayerCard")
AddEventHandler("np-poker:givePlayerCard", function(playerCardNum, cardNumber, faceUp)
    ...
    if playerCardObjects[1] ~= nil and playerCardObjects[2] ~= nil then
        TriggerEvent("np-poker:clearPlayerCards")
        Wait(100)
    end
...
RegisterNetEvent("np-poker:setPotAmount")
AddEventHandler("np-poker:setPotAmount", function(eventData)
    ...
    if tableNum ~= nil and potAmount ~= nil and prevPotAmounts[tableNum] ~= potAmount then
        if isSittingAtPokerTable then
            TriggerEvent("np-poker:clearPlayerBet")
        end
...
RegisterNetEvent("np-poker:clearPot")
AddEventHandler("np-poker:clearPot", function(eventData)
    local tableNum = tonumber(eventData.tableNum)
...
RegisterNetEvent("np-poker:setPlayerChips")
AddEventHandler("np-poker:setPlayerChips", function(chipCount)
    ...
    -- RPC.execute("np-poker:createNetworkedChips", objCoords.x, objCoords.y, objCoords.z, heading, chipCount, myTableId)
end)

-- Deletes player's cards
-- (idk if this will work networked, might need to set coords to 0,0,0?)
RegisterNetEvent("np-poker:clearPlayerCards")
AddEventHandler("np-poker:clearPlayerCards", function()
...
RegisterNetEvent("np-poker:clearPlayerBet")
AddEventHandler("np-poker:clearPlayerBet", function()
    for _, prop in pairs(currentBetObjects) do
        if prop ~= nil then
            DeleteObject(prop)
        end
...
-- RegisterNetEvent('np-poker:clearNetworkObjects')
-- AddEventHandler("np-poker:clearNetworkObjects", function(tableNum)
--   TriggerEvent("np-poker:clearNetworkCards", tableNum)
--   TriggerEvent("np-poker:clearNetworkBets", tableNum)
--   TriggerEvent("np-poker:clearNetworkChips", tableNum)
-- end)

-- Deletes the table cards for the specified table
RegisterNetEvent("np-poker:clearDealerCards")
AddEventHandler("np-poker:clearDealerCards", function(tableNum)
...
RegisterNetEvent("np-poker:playDealerEvent")
AddEventHandler("np-poker:playDealerEvent", function(eventData)
    ...
    -- Needed: playerPos
    -- After calling this event, you should send np-poker:givePlayerCard to the player getting the card
    ...
    -- Needed: playerPos
    -- After calling this event, you should send np-poker:clearPlayerCards to the specifed player
    ...
    if eventData.event == "clearCards" then
        ...
        Wait(1500)
        TriggerEvent("np-poker:clearDealerCards", pokerTable)
        Wait(1000)
        TaskPlayAnim(dealerPed, dealerAnimDictPoker, "deck_idle", 2.0, -2.0, -1, 13, 0.0, 0, 0, 0)
    end
...
RegisterNetEvent("np-poker:playPlayerEvent")
AddEventHandler("np-poker:playPlayerEvent", function(eventData)
    ...
    if eventData.event == "placeBet" then
        ...
        -- RPC.execute("np-poker:createNetworkedBet", objCoords.x, objCoords.y, objCoords.z, heading, betAmount, myTableId)
    end
    ...
    if eventData.event == "showCards" then
        ...
        TriggerEvent("np-poker:givePlayerCard", 1, cardNum1, true)
        TriggerEvent("np-poker:givePlayerCard", 2, cardNum2, true)
    end
    ...
    if eventData.event == "joinGame" then
        ...
        RPC.execute("np-casino:poker:joinGame", GetPlayerServerId(PlayerId()), pokerTable, playerPos)
        ...
        RPC.execute("np-poker:setChairActive", playerPos, pokerTable)
        Wait(3000)
        ...
        local chipCount = RPC.execute("np-casino:getCurrentChipCount")
        TriggerEvent("np-poker:setPlayerChips", chipCount)
        -- RPC.execute("np-casino:poker:joinGame", PlayerId(), pokerTable, playerPos)
    end
    ...
    if eventData.event == "leaveGame" then
        ...
        RPC.execute("np-casino:poker:playerAction", GetPlayerServerId(PlayerId()), myTableId, "leave", 0)
        myTableId = -1
        exports["np-ui"]:sendAppEvent("poker", {show = false, active = false})
        ...
        RPC.execute("np-poker:leaveChair", tonumber(currentPlayerPos), tonumber(currentPokerTable))
        -- RPC.execute("np-casino:poker:leaveGame", currentPokerTable, playerPos)
    end
end)

-- RegisterNetEvent("np-poker:createNetworkCard")
-- AddEventHandler("np-poker:createNetworkCard", function(position, rotation, cardNumber, tableNum)
...
-- RegisterNetEvent("np-poker:createNetworkBet")
-- AddEventHandler("np-poker:createNetworkBet", function(position, heading, betAmount, tableNum)
--     local tableObj = pokerTable1
--     if tableNum == 2 then tableObj = pokerTable2 end
--     tableObj.networkedBets[#tableObj.networkedBets + 1] = generateChipStackAtPosition(position, heading, betAmount, false)
-- end)

-- RegisterNetEvent("np-poker:createNetworkChips")
-- AddEventHandler("np-poker:createNetworkChips", function(position, heading, betAmount, tableNum)
--     local tableObj = pokerTable1
--     if tableNum == 2 then tableObj = pokerTable2 end
--     pposition)
...
-- RegisterNetEvent('np-poker:clearNetworkCards')
-- AddEventHandler("np-poker:clearNetworkCards", function(tableNum)
--     local tableObj = pokerTable1
--     if tableNum == 2 then tableObj = pokerTable2 end
...
-- RegisterNetEvent('np-poker:clearNetworkBets')
-- AddEventHandler("np-poker:clearNetworkBets", function(tableNum)
--     local tableObj = pokerTable1
--     if tableNum == 2 then tableObj = pokerTable2 end
...
-- RegisterNetEvent('np-poker:clearNetworkChips')
-- AddEventHandler("np-poker:clearNetworkChips", function(tableNum)
--     local tableObj = pokerTable1
--     if tableNum == 2 then tableObj = pokerTable2 end
...
function createCardAtPosition(positionVec, rotationVec, bool1, cardNum, networked)
    ...
    -- if networked then
    --   RPC.execute("np-poker:createNetworkedCard", cardNum, positionVec.x, positionVec.y, positionVec.z, rotationVec.x, rotationVec.y, rotationVec.z, myTableId)
    --   return
    -- end
    ...
    while true do
        ...
        if currentPokerTable and isSittingAtPokerTable then
            if not leaveTablePrompt then
                exports["np-ui"]:showInteraction("[E] Leave Table")
                leaveTablePrompt = true
            end
            if IsControlJustReleased(0, 38) then
                TriggerEvent("np-poker:playPlayerEvent", {event = "leaveGame"})
            end
        ...
        if currentPokerTable and not isSittingAtPokerTable then
            ...
            if #(GetEntityCoords(currentChairObject) - coords) < 2.0 and not pokerTablePrompt then
                ...
                exports["np-ui"]:showInteraction("[E] Play!", background)
            ...
            if IsControlJustPressed(0, 38) and canPlayAtCurrentTable then
                ...
                if chipCount < buyIn then
                    ...
                    local success = RPC.execute("np-poker:requestChair", tonumber(playerPos), tonumber(currentPokerTable))
                    if success then
                        TriggerEvent("np-poker:playPlayerEvent", {event = "joinGame", tableNum = currentPokerTable, seatNum = playerPos})
                    else
                        canPlayAtCurrentTable = true
...
--     TriggerEvent("np-poker:playDealerEvent", {event = args[1], tableNum = 1})
-- end, false)

-- RegisterCommand("PlayPlayerAnim", function(source, args, rawCommand)
--     if not args[1] or args[1] == nil then return end
--     TriggerEvent("np-poker:playPlayerEvent", {event = args[1], tableNum = 1})
-- end, false)
...
--     TriggerEvent("np-poker:playDealerEvent", data)
-- end, false)
...
--     if not args[1] or args[1] == nil then return end
--     TriggerEvent("np-poker:givePlayerCard", args[1], 1, false)
-- end, false)
...
--     if not args[1] or args[1] == nil then return end
--     TriggerEvent("np-poker:clearDealerCards", args[1])
-- end, false)

-- RegisterCommand("BetAmount", function(source, args, rawCommand)
--     if not args[1] or args[1] == nil then return end
--     TriggerEvent("np-poker:playPlayerEvent", {event = "placeBet", betAmount = args[1]})
-- end, false)

-- RegisterCommand("PotAmount", function(source, args, rawCommand)
--     if not args[1] or args[1] == nil then return end
--     TriggerEvent("np-poker:setPotAmount", {tableNum = 1, potAmount = args[1]})
-- end, false)

-- RegisterCommand("ShowCards", function(source, args, rawCommand)
--     if not args[1] or args[1] == nil then return end
--     if not args[2] or args[2] == nil then return end
--     TriggerEvent("np-poker:playPlayerEvent", {event = "showCards", cards = {args[1], args[2]}})
-- end, false)
...
AddEventHandler("np-casino:casinoExitedEvent", function()
    ...
    swappedPeds = {}
    swappedDecks = {}
    TriggerEvent("np-poker:clearPot", {tableNum = 1})
    TriggerEvent("np-poker:clearPot", {tableNum = 2})
    TriggerEvent("np-poker:clearPlayerCards")
    TriggerEvent("np-poker:clearPlayerBet")
    TriggerEvent("np-poker:clearDealerCards", 1)
    TriggerEvent("np-poker:clearDealerCards", 2)
end)
